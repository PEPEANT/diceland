...
/**
 * 移댁?????
 * ?쒖? ?명꽣?섏씠?? enter(), exit(), handleAction(action), update(dt), render(ctx)
 */
...
/**
    ...
    update(dt) {
        ...
        if (!this.slotOpen && !this.exchangeOpen && !this.blackjackOpen && !this.rouletteOpen) {
            ...
            this.player.move(direction, MAP_W, MAP_H, (x, y) => {
                // ?좏슚???꾩튂?몄? 寃??(異⑸룎?섎㈃ false)
                for (const o of this.objects) {
                    if (this._checkCollision(o, x, y)) {
                        return false; // 異⑸룎 諛쒖깮 -> ?대룞 遺덇?
                    }
                }
                return true; // ?덉쟾
            });
    ...
    _ensureFeatures() {
        ...
        if (!this.blackjackUI) {
            ...
            window.blackjackUI = this.blackjackUI; // ?붾쾭洹몄슜 ?몄텧
        }
    ...
    _openSlot() {
        ...
        if (this.slotGame) {
            ...
            this.slotGame.setMessage('?덈쾭瑜??밴꺼二쇱꽭??');
        }
    }
    ...
    _updateInteraction() {
        ...
        if (best) {
            ...
            const text =
                best.type === 'exchange' ? 'F를 눌러 환전소'
                : best.type === 'blackjack' ? 'F를 눌러 블랙잭'
                : best.type === 'roulette' ? 'F를 눌러 룰렛'
                : best.type === 'exit' ? 'F를 눌러 나가기'
                : 'F를 눌러 슬롯머신';
    ...
    /**
     * 媛앹껜 異⑸룎 ?뺤씤
     * @param {object} o - 異⑸룎???뺤씤??媛앹껜
     * @param {number} nextX - ?뚮젅?댁뼱???ㅼ쓬 X ?꾩튂
     * @param {number} nextY - ?뚮젅?댁뼱???ㅼ쓬 Y ?꾩튂
